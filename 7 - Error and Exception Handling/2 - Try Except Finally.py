...
# Now lets see what would happen if we did not have write
# permission (opening only with 'r'):
...
# Great! Notice how we only printed a statement!
# The code still ran and we were able to continue doing actions
# and running code blocks. This is extremely useful when you
# have to account for possible input errors in your code.
# You can be prepared for the error and keep running code,
# instead of your code just breaking as we saw above.
# We could have also just said except: if we weren't sure what
# exception would occur. For example:
...
try:
   ...
   f.close()
# Error: Could not find file or read data
# Great! Now we don't actually need to memorize that list of
# exception types! Now what if we kept wanting to run code
# after the exception occurred? This is where finally comes in.
...
# Lets see a new example that will take into account a user
# putting in the wrong input:
...
# assignment
# Notice how we got an error when trying to print val
# (because it was never properly assigned)
# Lets remedy this by asking the user and checking to make
...
# Hmmm...that only did one check. How can we continually keep
# checking? We can use a while loop!
...
# Finally, I executed!
# Great! Now you know how to handle errors and exceptions in
# Python with the try, except, else, and finally notation!